...
using Forked.Dynamic.Utils;
using Forked.Linq.Expressions;
using Forked.Runtime.CompilerServices;

using DelegateHelpers = Forked.Linq.Expressions.Compiler.DelegateHelpers;

namespace Forked.Dynamic
{
    /// <summary>
    ...
    public abstract class DynamicMetaObjectBinder : CallSiteBinder
    {
        ...
        public sealed override Expression Bind(object[] args, ReadOnlyCollection<ParameterExpression> parameters, LabelTarget returnLabel)
        {
            ...
            if (args.Length == 0)
            {
                throw Forked.Linq.Expressions.Error.OutOfRange("args.Length", 1);
            }
            if (parameters.Count == 0)
            {
                throw Forked.Linq.Expressions.Error.OutOfRange("parameters.Count", 1);
            }
            ...
            if (IsStandardBinder)
            {
                ...
                {
                    throw Forked.Linq.Expressions.Error.BinderNotCompatibleWithCallSite(expectedResult, this, returnLabel.Type);
                }
            }
            else
            {
            ...
            if (binding == null)
            {
                throw Forked.Linq.Expressions.Error.BindingCannotBeNull();
            }
                ...
                !TypeUtils.AreReferenceAssignable(expectedResult, body.Type))
            {
                ...
                {
                    throw Forked.Linq.Expressions.Error.DynamicObjectResultNotAssignable(body.Type, target.Value.GetType(), this, expectedResult);
                }
                else
                {
                    throw Forked.Linq.Expressions.Error.DynamicBinderResultNotAssignable(body.Type, this, expectedResult);
                }
            }
            ...
            if (IsStandardBinder && args[0] as IDynamicMetaObjectProvider != null)
            {
                ...
                {
                    throw Forked.Linq.Expressions.Error.DynamicBindingNeedsRestrictions(target.Value.GetType(), this);
                }
            }
