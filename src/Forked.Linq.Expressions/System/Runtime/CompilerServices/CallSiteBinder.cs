    ...
    public abstract class CallSiteBinder
    {
        ...
        private sealed class LambdaSignature<T> where T : class
        {
            ...
            private LambdaSignature()
            {
                ...
                {
                    throw Forked.Linq.Expressions.Error.TypeParameterIsNotDelegate(target);
                }
                ...
                {
                    throw Forked.Linq.Expressions.Error.FirstArgumentMustBeCallSite();
                }
        ...
        internal T BindCore<T>(CallSite<T> site, object[] args) where T : class
        {
            ...
            if (binding == null)
            {
                throw Forked.Linq.Expressions.Error.NoOrInvalidRuleProduced();
            }
        ...
        private static Expression<T> Stitch<T>(Expression binding, LambdaSignature<T> signature) where T : class
        {
            ...
            var body = new System.Runtime.CompilerServices.ReadOnlyCollectionBuilder<Expression>(3);
            body.Add(binding);
