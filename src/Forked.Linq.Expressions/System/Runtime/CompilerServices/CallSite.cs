...
using Forked.Dynamic;
using Forked.Dynamic.Utils;
using Forked.Linq.Expressions;
using System.Reflection;
using System.Threading;
using System.Runtime.CompilerServices;
using Forked.Collections.Generic;
using static Forked.Linq.Expressions.CachedReflectionInfo;

namespace Forked.Runtime.CompilerServices
{
    //
    ...
    public class CallSite
    {
        ...
        public static CallSite Create(Type delegateType, CallSiteBinder binder)
        {
            ...
            if (!delegateType.IsSubclassOf(typeof(MulticastDelegate))) throw Forked.Linq.Expressions.Error.TypeMustBeDerivedFromSystemDelegate();
    ...
    public class CallSite<T> : CallSite where T : class
    {
        ...
        {
            if (!typeof(T).IsSubclassOf(typeof(MulticastDelegate))) throw Forked.Linq.Expressions.Error.TypeMustBeDerivedFromSystemDelegate();
            ContractUtils.RequiresNotNull(binder, nameof(binder));
        ...
        internal T MakeUpdateDelegate()
        {
            ...
            if (target.IsGenericType && IsSimpleSignature(invoke, out args))
            {
                ...
                {
                    if (target == Forked.Linq.Expressions.Compiler.DelegateHelpers.GetActionType(args.AddFirst(typeof(CallSite))))
                    {
                ...
                {
                    if (target == Forked.Linq.Expressions.Compiler.DelegateHelpers.GetFuncType(args.AddFirst(typeof(CallSite))))
                    {
